...
class Wheel extends cp.Body {
    spec: any;

    constructor(space, spec) {
        super(
            spec.mass, 
        ...
        let shape = new cp.CircleShape(this, spec.radius, v(0,0));
    ...
    draw(ctx, viewbox, res) {
        let r = this.spec.radius;
...
export class Vehicle {
    ...
    private chassis: cp.Body;
    private frontWheel: Wheel;
    private backWheel: Wheel;
    private driver: cp.Body;

    private frontMotor: cp.SimpleMotor;
    private backMotor: cp.SimpleMotor;
    private differential: cp.SimpleMotor;

    get p(): cp.Vect {
        return this.chassis.p;
    }

    constructor(space, spec, offset) {
        this.spec = spec;
        ...
        var makeChassis = function(space, spec) {
            ...
            var bodyShape = new cp.PolyShape(
                ...
                cp.vmult(spec.body_outline_offset, spec.body_outline_scale));
            ...
            var cabShape = new cp.PolyShape(
                ...
                cp.vmult(spec.cab_outline_offset, spec.body_outline_scale));
